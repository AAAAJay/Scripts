...
namespace Boss {
	public class BasicBoss : MoveClass {
        ...
        public BossRandomClass[] BossRandoms = new BossRandomClass[0];//射擊偏差值
        ...
        IEnumerator AnimTime(float waitTime)
        {
            yield return new WaitForSeconds(waitTime);
        ...
        public float BossRandom(string name)
        {
            float value = 0;
            foreach (BossRandomClass BossRandomV in BossRandoms)
            {
                if (BossRandomV.ValueName == name)
                {
                    value = BossRandomV.FireRandom();
                }
            }
            return value;
        }
        ...
        public Vector3 TargetData()
        {
            ...
            targetDate = new Vector3(distance, RotaF, RotaR);
            return targetDate;
        }
