...
public class MouseRotation : MonoBehaviour 
{
    ...
    void Update()
    {
        ...
        {
            /*foreach (GameObject objj in LM.monters)
            {
            ...
            else
            {
                ...
                if (LM.monters.Length != 0)
                {
                    foreach (GameObject gg in LM.monters)
                    {
                        ...
                        float k = KKKKK.DistanceOfPointToVector(camera.transform.position, camera.transform.position + (camera.transform.forward * lockD), gg.transform.position);
                        if (k < D)
                        {
                            if (gg.GetComponent<Boss.BasicBoss>() == false)
                            {
                                LM.mm.Remove(gg);
                            }
                            else
                            ...
                            //ttt.transform.position = new Vector3(camera.transform.position.x, camera.transform.position.y, camera.transform.localPosition.z + lockD);
                            //ttt.transform.position = camera.transform.position + (camera.transform.forward*lockD);
                            D = k;
                            kkkkk = k;
                            g = gg.transform;
                        }
        ...
        if( g.tag != "Player" && g.GetComponent<Boss.BasicBoss>()==false)
        {
            lockkk = false;
            LockUi.gameObject.SetActive(false);
            g = Ani.transform;
        ...
        if (Vector3.Distance(g.transform.position, transform.position)>lockD)
        {
            lockkk = false;
            LockUi.gameObject.SetActive(false);
            g = Ani.transform;
